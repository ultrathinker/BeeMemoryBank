...
public class ArticleVersionRepository(DbConnectionFactory factory) : BaseRepository(factory), IArticleVersionRepository
{
    public async Task<List<ArticleVersion>> GetByArticleIdAsync(Guid articleId)
    {
        ...
        var rows = await conn.QueryAsync(
            @"SELECT id AS Id, article_id AS ArticleId, version_number AS VersionNumber,
                     title AS Title, tags AS TagsJson, tree_path AS TreePath,
        ...
        return rows.Select(r => new ArticleVersion
        {
            ...
            Tags = ParseTags((string)r.TagsJson),
    ...
    public async Task CreateAsync(ArticleVersion version)
    {
        ...
        await conn.ExecuteAsync(
            @"INSERT INTO tbl_article_version
              (id, article_id, version_number, title, tags, tree_path, ciphertext, iv, encrypted_dek, dek_iv, updated_by, created_at)
              VALUES (@Id, @ArticleId, @VersionNumber, @Title, @TagsJson, @TreePath, @Ciphertext, @IV, @EncryptedDek, @DekIV, @UpdatedBy, @CreatedAt)",
            new
            {
                version.Id,
                version.ArticleId,
                ...
                version.Title,
                TagsJson = JsonSerializer.Serialize(version.Tags),
                version.TreePath,
                version.Ciphertext,
                version.IV,

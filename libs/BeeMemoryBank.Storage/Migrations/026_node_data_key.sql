-- Creates the GENERIC table tbl_node_data_key. chat.db is its first (and today only) user, but the
-- DEK-rotation rewrap that re-seals these rows lives in BeeMemoryBank.Sync, which must not know about
-- chat (ChatIsolationGuardTests), so the table is keyed by name and the rewrap walks every row without
-- knowing what each one protects.
--

...
public static class SegmentLayout
{
    ...
    /// <summary>The only format version this code produces/understands.</summary>
    ...
    // Carries termTextOffset/termTextLength so a reader can disambiguate a hash collision (two
    // distinct terms sharing a 64-bit hash) by exact byte comparison against the term text block,
    // rather than treating the hash alone as a unique key. postingsOffset and termTextOffset are
    // absolute byte offsets from the start of the whole segment (not block-relative), so a reader
    // never has to track where each block begins. See SegmentWriter's XML doc for the rationale.

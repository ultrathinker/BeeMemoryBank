...
internal sealed class XlmRobertaTokenizer
{
    ...
    internal static XlmRobertaTokenizer LoadDefault()
    {
        var modelStream = typeof(XlmRobertaTokenizer).Assembly
            .GetManifestResourceStream("BeeMemoryBank.Embeddings.Models.sentencepiece.bpe.model")
            ?? throw new InvalidOperationException("Embedded sentencepiece.bpe.model not found in BeeMemoryBank.Embeddings assembly.");

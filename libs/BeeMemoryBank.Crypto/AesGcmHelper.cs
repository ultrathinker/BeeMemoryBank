...
internal static class AesGcmHelper
{
    ...
    /// </summary>
    internal static (byte[] ciphertextWithTag, byte[] iv) Encrypt(byte[] key, byte[] plaintext)
    {
    ...
    /// </summary>
    internal static byte[] Decrypt(byte[] key, byte[] ciphertextWithTag, byte[] iv)
    {
        ...
        using var aes = new AesGcm(key, CryptoConstants.TagSize);
        aes.Decrypt(iv, ciphertextOnly, tag, plaintext);

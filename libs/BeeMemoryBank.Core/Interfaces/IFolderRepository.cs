...
public interface IFolderRepository
{
    ...
    /// (the default) means the whole vault; a non-root value additionally restricts to that folder and its descendants, mirroring
    ...
    /// leaf themselves (see <see cref="ThrowIfWriteDenied"/>) BEFORE calling it; skipping that lets
    /// a denied caller litter restricted subtrees with folders.
    /// </para>
    ///
    /// <para>
    ...
    /// <summary>
    /// Legacy exact-substring search, from before the FTS5 index (per-row <c>unicode_contains</c> scan over name and path,

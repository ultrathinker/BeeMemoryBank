...
public class CallerScopeHolder
{
    ...
    /// in-memory InstanceCallerScopeStore (plain per-instance storage).
    /// </summary>
    ...
    ///
    /// <para>Do not hand-write elevation (assign <c>Scope = SystemCallerScope.Instance</c>, then a
    /// try/finally that puts the old value back). Forgetting the finally does not fail visibly — it
    /// leaks full-vault access to whatever runs next on the same scope store, and under

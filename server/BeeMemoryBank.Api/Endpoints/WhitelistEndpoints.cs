...
public static class WhitelistEndpoints
{
    public static void MapWhitelistEndpoints(this WebApplication app)
    {
        ...
        // POST /api/whitelist — add a node (requires unlock)
        group.MapPost("/", async (
            AddWhitelistEntryRequest req,
            ...
            HttpContext ctx) =>
        {
            ...
            if (await repo.GetByNodeIdAsync(req.NodeId, includeDeleted: true) != null)
                return Results.Conflict(new ErrorResponse($"Node {req.NodeId} already in whitelist"));
            ...
            var entry = new WhitelistEntry
            {
                ...
                CanGenerateEmbeddings = req.CanGenerateEmbeddings,
                Status = "A",
                CreatedAt = now,
                UpdatedAt = now
            };

            await repo.CreateAsync(entry);
            await eventLogger.LogWhitelistAddAsync(entry);

            return Results.Created($"/api/whitelist/{entry.NodeId}", WhitelistEntryResponse.From(entry));
        });

...
$(function () {
    ...
    if (treeContainer) {
        ...
        treeContainer.addEventListener('click', function (e) {
            ...
            // If the main panel shows this same folder, re-fetch the right side via SPA
            // navigation (same swap path as clicking the folder itself). The tree subtree
            // is refreshed unconditionally below — both panels stay in sync.
    ...
    // ===== Reusable folder picker (search input + Ajax dropdown) =====
    // Markup the caller provides inside `rootEl`:
    //   <sl-input class="bmb-fp-input" placeholder="..." autocomplete="off">
    //   <div class="bmb-fp-dropdown" hidden></div>
    //
    // Options: { rootEl, initialPath, onChange(path) }
    // Returns: { setPath(p), getPath(), focus() }
    //
    ...
    // Single global outside-click handler for ALL folder pickers — avoids
    // per-picker document listeners that pile up under SPA navigation
    // (the old DOM is detached but the listener closures keep it alive).
    ...
    window.bmbFolderPicker = function(opts) {
        ...
        if (!state) {
            ...
            // ARIA combobox/listbox wiring so screen readers announce the
            ...
            // (mouseover/out removed — using CSS :hover on .bmb-fp-item now.)
            // Outside-click close is handled by the global listener above.
        }
    ...
    // ===== Path-selector helper (used inside create-article/folder dialogs) =====
    // Renders the compact "/Some/Path  ✏️" header. When the user clicks the
    // path or the pencil, a folder picker (search + dropdown) reveals
    // underneath, letting them pick ANY folder including root. When there's no
    // selected path (sidebar with no tree selection), the pencil is hidden and
    // the line just shows "/" — but the picker can still be opened to choose.
    //
    // The DOM elements are persistent (created once in the .cshtml) so this
    // helper attaches listeners on the FIRST call and just refreshes state on
    // subsequent calls — re-binding every dialog open would accumulate
    // listeners and break the toggle.
    //
    // Options: { rootEl, basePath }. Returns: { getEffectivePath() }.
    ...
    // New Article: no popup at all. Jump straight to the editor with the
    // currently-selected folder (or root) as the initial treePath. The user
    // can change the path via the clickable breadcrumb on the editor page.
    var newArticleLink = document.getElementById('btn-new-article-sidebar');
    if (newArticleLink) {
        newArticleLink.addEventListener('click', function(e) {
            e.preventDefault();
            var basePath = window.bmbGetSelectedFolderPath() || '/';
    ...
    if (addFolderLink && addFolderDlg) {
        addFolderLink.addEventListener('click', function(e) {
            e.preventDefault();
            _sidebarSelectedPath = window.bmbGetSelectedFolderPath();

...
The project includes 32 Maestro YAML tests in `mobile/maestro-tests/`. These run against a physical Android device with a BeeMemoryBank test node.

```bash
# Run a single test
~/.maestro/bin/maestro test mobile/maestro-tests/<test>.yaml

# Run all tests (~39 min)
~/.maestro/bin/maestro test mobile/maestro-tests/
```
...
│   ├── BeeMemoryBank.Web/     Razor Pages frontend (proxies to Api)

...
public class SchemaTests : IAsyncLifetime
{
    ...
    [Fact]
    public async Task TblTag_InsertAndSelect()
    {
        using var conn = _factory.CreateConnection();
        await conn.ExecuteAsync("INSERT INTO tbl_tag (name) VALUES (@name)", new { name = "dotnet" });
        var name = await conn.QuerySingleAsync<string>("SELECT name FROM tbl_tag WHERE name = 'dotnet'");
        name.Should().Be("dotnet");
    }

    [Fact]
    public async Task TblArticleTag_InsertAndSelect()
    {
        using var conn = _factory.CreateConnection();
        ...
        await conn.ExecuteAsync("INSERT INTO tbl_tag (name) VALUES ('tag1')");
        var tagId = await conn.QuerySingleAsync<int>("SELECT id FROM tbl_tag WHERE name = 'tag1'");
        await conn.ExecuteAsync(
            "INSERT INTO tbl_article_tag (article_id, tag_id) VALUES (@articleId, @tagId)",
            new { articleId, tagId });

        var count = await conn.QuerySingleAsync<int>(
            "SELECT COUNT(*) FROM tbl_article_tag WHERE article_id = @articleId", new { articleId });
        count.Should().Be(1);
    }

...
public class MigrationTests : IAsyncLifetime
{
    ...
    public async Task RunMigrations_AppliesWithoutErrors()
    {
        ...
        var version = await conn.QuerySingleAsync<int>("SELECT version FROM tbl_migration WHERE version = 1");
        version.Should().Be(1);
    }
    ...
    public async Task Schema_AllTablesExist()
    {
        ...
        tables.Should().Contain("tbl_article_tag");

...
public class InitializationTests : TestFixture
{
    ...
    {
        await InitService.InitializeAsync("admin", "Desktop", "myPassword");
    ...
    {
        await InitService.InitializeAsync("admin", "Node1", "pass");

        var act = async () => await InitService.InitializeAsync("admin", "Node2", "pass2");

...
public class TreeSearchTests : TestFixture
{
    ...
    [Fact]
    public async Task Search_ByTag_FindsArticles()
    {
        var results = await SearchService.SearchAsync("devops");
        results.Articles.Should().HaveCount(2);
        results.Articles.Should().OnlyContain(a => a.Tags.Contains("devops"));
    }

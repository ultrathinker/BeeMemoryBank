...
- **Production-grade.** Survived multiple rounds of independent security review (crypto, sync/trust model, auth, input, admin, hygiene, mobile) — every finding either fixed or explicitly documented as accepted-risk. **1,500+ tests pass on every build.** Confidential per-peer DEK rotation, snapshot/restore, hard-delete propagation, encrypted version history, audit log. Code is open under AGPL-3.0; nothing hidden.

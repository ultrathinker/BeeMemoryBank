...
| Role-based access | Two system roles (`superadmin`, `user`) plus any number of custom roles, all in `tbl_role`. Superadmins manage users and roles, unlock the vault, and bypass every folder restriction; every other role is unprivileged — each authorization check tests for the literal `"superadmin"` — and differs only in the folder rules attached to it. Regular users can only log in when the vault is already unlocked by a superadmin or agent |

...
- **Результат после фикса:** резко возросшая надёжность (было near-0% при повторных прогонах подряд → стало ~80-90% в повторных прогонах). **Остаточная, более редкая нестабильность (~1 из 5-8 прогонов) всё ещё наблюдается** — вероятно отдельная, более узкая гонка, не идентифицированная до конца. **Это не блокирует Этап 1** (фикс путей данных, полностью ортогонален) — но **должно быть доведено до полной надёжности до начала Этапа 4** (переключение хранилищ напрямую зависит от надёжности graceful-stop). Помечено как открытый пункт, не потерян молча.
